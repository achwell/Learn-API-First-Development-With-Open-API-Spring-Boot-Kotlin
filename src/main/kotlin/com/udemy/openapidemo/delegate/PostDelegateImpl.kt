...
@Service
class PostDelegateImpl(private val postRepository: PostRepository): PostDelegate {
    ...
    override fun addPost(addPostRequestBody: AddPostRequestBody): ResponseEntity<AddPostResponse> {
        ...
        return runCatching {
            ...
            onSuccess = {
                val postId = it.id ?: return ResponseEntity(INTERNAL_SERVER_ERROR)
                Logger.info("Post Id {}", postId)
                ResponseEntity(AddPostResponse(postId), CREATED)
            },
            onFailure = {
    ...
    override fun getAllPosts(): ResponseEntity<GetPostsResponse> {
        ...
        return ResponseEntity(GetPostsResponse(postDtos), OK)
    }
